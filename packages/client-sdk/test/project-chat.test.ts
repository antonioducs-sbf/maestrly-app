...
it('deduplicates deltas, detects gaps and isolates sessions', () => {
  ...
  const state={session:{id:sessionId},cursor:0,messages:[{id:messageId,parts:[]}],interactions:[],turn:null} as unknown as ProjectChatSnapshot
  const event={version:1,sessionId,sequence:1,eventId:'one',payload:{type:'delta',messageId,partId:'text',kind:'text',delta:'Olá'}} as ProjectChatEvent
...
it('decodes UTF-8 split across network chunks and CRLF SSE frames', async () => {
  const event={version:1,sessionId:crypto.randomUUID(),sequence:1,eventId:'one',payload:{type:'delta',messageId:crypto.randomUUID(),partId:'text',kind:'text',delta:'Olá 🚀'}}
  ...
  const received=[];for await(const value of readProjectChatEvents(new Response(stream)))received.push(value)
