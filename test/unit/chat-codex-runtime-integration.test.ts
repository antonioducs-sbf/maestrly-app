...
const packageRoot = target
  ? path.join(root, 'node_modules', ...target.optionalPackage.split('/'))
  : path.join(root, 'node_modules', '__unsupported__')
...
describe.skipIf(!target || !existsSync(expectedBinary))('official Codex runtime', () => {
  ...
  it('neutralizes native multi-agent catalogs independently of feature flags', async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({ fetched_at: new Date().toISOString(), client_version: '0.153.2', models }),
        'utf8'
      )
  ...
  it('retains cross-version gates after upstream fixture corrections', async () => {
    ...
    try {
      ...
      expect(runtimeVersion).toBe('0.153.2')
