...
4. Open **Chat** in the project's web header. Choose executor, workspace, model, base branch and mode, then **Start conversation**. **Read-only chat** does not permit code or board changes; **Agent** can request permission to change them.
...
Deployment requires migration `008_project_chat.sql`, the API, the web bundle and an updated executor. Roll out the additive migration/API first, then web and the executor. Preserve database backup and prior images; a web-only rollout does not deliver this feature. See [self-hosting](self-hosting.md) and [desktop executor](desktop-executor.md).

Run `node scripts/test-project-chat-e2e.mjs` to validate PostgreSQL, API, Electron, a local deterministic model, a fixture MCP and Chromium together. It exercises code/memory/skill/card context, multi-turn history, streaming, web interactions and reconnection without paid model calls.
