...
export function OperationsPanel({ organizationId, projectId }: { organizationId: string; projectId: string }) {
  ...
  return <section className="table-panel">
    ...
    {operations.length === 0 ? <div className="empty"><Activity /><h3>{t("No execution yet")}</h3><p>{t("Move a card into an agentic column to create one authorized job.")}</p></div> : <div className="operation-table" role="table">
