...
it('rejects remote global permission and internal messages', () => {
  ...
  expect(projectChatMessageSchema.safeParse({id:crypto.randomUUID(),sessionId:crypto.randomUUID(),turnId:null,role:'system',parts:[],createdAt:new Date().toISOString()}).success).toBe(false)
