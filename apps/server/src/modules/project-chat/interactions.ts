...
export async function decideInteraction(c:DatabaseClient,s:ProjectChatSession,id:string,version:number,decision:ChatDecision) {
  const row=(await c.query('select * from chat_interactions where session_id=$1 and id=$2 for update',[s.id,id])).rows[0]
  if(!row)chatFail('Interaction not found.',404)
  const interaction=mapInteraction(row)
  if(interaction.version!==version || interaction.payload.type!==decision.type)chatFail('This interaction has changed.')
  ...
  } else if(!['running','waiting_input'].includes(turn.state)||!turn.leaseExpiresAt||Date.parse(turn.leaseExpiresAt)<=Date.now())chatFail('The execution was interrupted.')
  if(decision.type==='question'&&interaction.payload.type==='question'&&decision.answers.length!==interaction.payload.questions.length)chatFail('Answer each question.',400)
  ...
  await appendChatEvent(c,s,{type:'interaction',interaction:resolved},'decision-'+id+'-'+version,interaction.turnId)
  if(decision.type==='plan'&&decision.action!=='discard') {
    const text=decision.action==='approve'?'Implement the approved plan.':'Revise the plan: '+decision.feedback
